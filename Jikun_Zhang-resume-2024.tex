...
\section{Experience}
  \resumeSubHeadingListStart
    ...
    \resumeProjectHeading
      ...
      \resumeItemListStart
        \resumeItem{C++ connectivity developer working on integrating Multi Commodity Exchange's market data and execution protocols.}
        \resumeItem{Built an application that ingest sniffed packet capture data and output them in Apache parquet format. Used Boost.PFR's structure reflection features to programmatically generate parquet tables and columns based on serialized layout of protocol message types. Doing so greatly improved capabilities compared to previous implementations that hard-code each member.}
        \resumeItem{Developed an application that reads sniffed packet capture data live, tracks order trade ratio and raises alarms when limit has been breached before any penalties are incurred.}
        \resumeItem{Utilised ef\_vi API to publish UDP unicast packets to local vlan containing session keys so sniffing applications can pick up on it and decrypt the encrypted protocol data.}
        \resumeItem{Designed the most profitable and overall best trading algorithm on a mock exchange with participants in training program.}
      \resumeItemListEnd
